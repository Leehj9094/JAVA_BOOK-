...
public class C03FunctionalInterfaceMain {
	...
	public static Function<List<Object>, Integer> func4=(li)->{
		...
		return li.stream()
			.filter((item) -> {return item instanceof Integer;})				// 1.Integer형만 필터링(instanceof)
			.map((item)->{
//				System.out.println(item);
				return (Integer)item;})													// 자료형 재구성(Object -> Integer)
			.reduce(0, (sum,item)->{return 0;});								// 누적합 (reduce)
		};
	// 05
	...
	// 08 
	public static Function<List<Integer>, Integer> subFunc = (x)->{return x.stream().reduce(1, (sum,x1)->sum+x1);};
	...
	public static void main(String[] args) {	
		...
		// 04
		System.out.println(func4.apply(Arrays.asList("홍길동",10,20,30,40.5,true)));
		// 05
		System.out.println(func5.apply(Arrays.asList("홍길동",10,20,30,40.5,true)));
		// 06
