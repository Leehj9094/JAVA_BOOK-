...
//		options.addArguments("--no-sandbox");			// 리눅스환경 sandbox 사용여부
		...
		// 키워드 입력
		searchEl.sendKeys("노트북"); // 적혀있는 것이 자동적으로 검색

		// 엔터키 전달
		searchEl.sendKeys(Keys.RETURN);

		// 노트북 검색 이후에 쇼핑 버튼 클릭
		WebElement shoppingEtnEl = driver.findElement(By.cssSelector(".api_pcpg_wrap .tab:nth-child(1)"));
		// target_blank(새창열기) 제거
		JavascriptExecutor js = (JavascriptExecutor) driver;
		js.executeScript("arguments[0].removeAttribute('target')", shoppingEtnEl);
